...
def breed(p1, p2):
    # given p1 and p2, produce an offspring with characteristics from both there is an equal chance of inheriting any
    # characteristic
...
class Particle:
    ...
    def update_score(self, env):
        ...
        if (self.checkpoints_passed + 1) % len(env.checkpoints) == 1 and \
                self.score > 0 and (env.time_elapsed - self.stopwatch) > 5000:

            if (env.time_elapsed - self.stopwatch) < self.fastest_lap:
                self.fastest_lap = env.time_elapsed - self.stopwatch

                print(
                    'Fastest lap! ' + str(round(self.fastest_lap * 100) / 100000) + 's for particle ' + str(self.name))
...
class Environment:
    ...
    def addParticles(self, n=1, **kargs):
        ...
        for i in range(n):
            ...
            particle = Particle(x, y, size, mass, accn=acceleration, turning_angle=turning_angle,
