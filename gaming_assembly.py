    ...
    while n < n_generations:
        ...
        while current_time - start_time < duration and running == True:
            ...
            for p in env.particles:
                ...
                if lines:
                    pygame.draw.line(screen, p.colour, (int(p.x), int(p.y)), (
                        int(p.x) + p.distance_front * math.sin(angle), int(p.y) + p.distance_front * math.cos(angle)))
                    pygame.draw.line(screen, p.colour, (int(p.x), int(p.y)), (
                        int(p.x) + p.distance_left * math.sin(angle + fov),
                        int(p.y) + p.distance_left * math.cos(angle + fov)))
                    pygame.draw.line(screen, p.colour, (int(p.x), int(p.y)), (
                        int(p.x) + p.distance_right * math.sin(angle - fov),
                        int(p.y) + p.distance_right * math.cos(angle - fov)))
...
if Race:
    ...
    for p in driver_list:
        fov = random.uniform(0, 90)
        env.addParticles(1, x=starting_grid[i][0], y=starting_grid[i][1], speed=0, size=5, control_rods=p.control_rods,
                         bias=p.bias, fov=p.fov)
        i += 1
    particle_list = env.particles
